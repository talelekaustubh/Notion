...
const Instructor = () => {
    ...
    useEffect(()=> {
        const getCourseDataWithStats = async() => {
            ...
            if(instructorApiData.length)
               await setInstructorData(instructorApiData);
    ...
    if(instructorData!==null){
        totalAmount=instructorData.reduce((acc,curr)=> acc + curr.totalAmountGenerated, 0);
  ...
  return (
    <div>
      ...
      <div className="relative space-y-2 ml-[300px] mt-4 mb-3">
        <h1 className="text-2xl font-bold text-richblack-5">Hi {user.firstName}</h1>
        <p className="font-medium text-richblack-200">Let's start something new</p>
     

      {loading ? (<div className='mt-10'><div className='spinner'></div></div>)
      :courses.length > 0 
        ...
        <div className="rounded-md bg-richblack-800 p-6 w-[1180px]">
            {/* Render 3 courses */}
